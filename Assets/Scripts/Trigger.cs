...
public class Trigger : MonoBehaviour
{
    ...
    {
        if (other.tag == "Player1" && engineController.engineID == 0)
        {
            textCanvas.enabled = true;
            interactImage.SetActive(true);
        }
        else if (other.tag == "Player2" && engineController.engineID == 1)
        {
            textCanvas.enabled = true;
            interactImage.SetActive(true);
        }
    ...
    {
        if (other.tag == "Player1" && Input.GetButtonDown("Y1") && timer >= interactDelay && engineController.engineID == 0)
        {
            Debug.Log("Pressed");
            timer = 0;
            engineController.SpeedControl();
            //interactText.text = "";
        }
        else if (other.tag == "Player2" && Input.GetButtonDown("Y2") && timer >= interactDelay && engineController.engineID == 1)
        {
            timer = 0;
            engineController.SpeedControl();
            //interactText.text = "";
        }
        else if(other.tag == "Player1" && Input.GetButtonDown("B1") && timer >= interactDelay && engineController.engineID == 0)
        {
            timer = 0;
            engineController.EnginePower();
            Debug.Log("B Pressed");
        }
        else if (other.tag == "Player2" && Input.GetButtonDown("B2") && timer >= interactDelay && engineController.engineID == 1)
        {
            timer = 0;
            engineController.EnginePower();
        }
